...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = te.Linear(config.hidden_size, config.vocab_size, bias=False)
...
class BertOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.seq_relationship = te.Linear(config.hidden_size, 2)
...
class BertForMultipleChoice(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(classifier_dropout)
        self.classifier = te.Linear(config.hidden_size, 1)
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModel(config, add_pooling_layer=False)
        self.qa_outputs = te.Linear(config.hidden_size, config.num_labels)
